...
- **模型可见** —— `rheostat:style` 提示片段（order 40，位于人格 0 之后、plan-mode 指导 50 之前）按会话语言渲染折叠位置及其区间的风格指令（≤ 0.25 为极简 0 模式，≥ 0.75 为饱满 1 模式，中间为混合）——`detectLanguage` 将最近一条用户消息分类为中文或英文，尚无用户消息时默认中文。plan-mode 规则因渲染在后，在计划评审期间仍可覆盖变阻器。
