...
Start the Web UI with `dsh web` (the browser surface is an explicit entry point; bare `dsh` opens the TUI). The command prints its URL, served at `http://127.0.0.1:3080` by default. This guide begins after that server is running. The `dsh` process uses its invoking directory as the default filesystem location, but a fresh Web UI has no selected workspace until you add one.
