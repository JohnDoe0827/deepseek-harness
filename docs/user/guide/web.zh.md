...
使用 `dsh web` 启动 Web UI（浏览器界面是显式入口；裸 `dsh` 打开 TUI）。命令会打印其访问地址，默认在 `http://127.0.0.1:3080` 提供服务。本指南从服务器已经运行的状态开始。`dsh` 进程会把调用目录作为默认文件系统位置，但新的 Web UI 在添加工作区前不会选中任何工作区。
