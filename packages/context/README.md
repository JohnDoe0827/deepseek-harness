...
Product plugins that add model-visible request context without defining a tool. `agent-instructions` is included by the default `dsh-agent-spine-demo` bundle and can be disabled through bundle config; `time-context`, `tmux-context`, `session-reference`, and `rheostat` are opt-in (the shipped base bundle mounts `rheostat` by default).
...
| [`rheostat/`](rheostat/README.md) | A per-session style dial (0..1) sliding response style between terse and expressive | — |
