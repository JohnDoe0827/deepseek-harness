...
The position is logged as a `rheostat/position` session event (whole-value replace, last write wins), so resume and fork restore it without a live mirror. The dial-state events carry the envelope's `ignorable` marker: a harness whose generated event vocabulary lacks this plugin's types (an out-of-repo install) skips them and keeps the session loadable instead of refusing the log, while a harness that knows the types folds them normally and restores the dial. Anyone can slide it:
...
- **Out-of-repo event vocabulary** — the dial-state events (`rheostat/position`, `rheostat/active`) are this plugin's own `SessionEventMap` members, outside a stock harness's generated known-event set. They append with the `ignorable` envelope marker (via `Session.append`'s `AppendOptions` flag), so a reader without the types keeps the session loadable and simply does not fold the dial; installing the plugin on a harness release that predates that append surface still records the events unmarked, and such sessions refuse to resume under first-party readers.
