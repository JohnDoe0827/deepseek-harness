...
位置以 `rheostat/position` 会话事件落盘（整值替换，最后一次写入生效），因此恢复与派生（fork）都能还原它，无需实时镜像。变阻器状态事件带有信封上的 `ignorable` 标记：生成的事件词汇表缺少本插件类型的 harness（仓库外安装）会跳过它们、保持会话可加载而不是拒绝日志；认识这些类型的 harness 则照常折叠并恢复变阻器。任何人都可以滑动它：
...
- **仓库外事件词汇表** — 变阻器状态事件（`rheostat/position`、`rheostat/active`）是本插件自己的 `SessionEventMap` 成员，不在发行版 harness 的生成已知事件集里。它们以信封上的 `ignorable` 标记追加（通过 `Session.append` 的 `AppendOptions` 标志）：不认识这些类型的读取器会跳过它们、保持会话可加载，只是不折叠变阻器状态；而认识这些类型的读取器照常折叠并恢复变阻器。若把插件安装到早于该 append 表面的 harness 版本上，事件仍会不带标记地记录，此类会话在第一方读取器下将拒绝恢复。
