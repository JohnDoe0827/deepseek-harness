 ...
 * The dial-state events append with the envelope's `ignorable` marker, so a
 * harness whose generated event vocabulary lacks this plugin's types (an
 * out-of-repo install) keeps the session loadable instead of refusing the
 * log. The model slides the dial through `rheostat_set` (and reads it through
...
export function apply(ctx: Context): void {
  ...
  function commitPending(session: Session, target: DialState): void {
    ...
    if (activeChanged) session.append('rheostat/active', { active: target.active }, { ignorable: true })
    if (positionChanged) session.append('rheostat/position', { position: target.position }, { ignorable: true })
    ...
    commandCtx.commands.register({
      ...
      handler: ({ agent, rawInput }) => {
        ...
        if (target.active !== foldActive(agent.session.events)) {
          agent.session.append('rheostat/active', { active: target.active }, { ignorable: true })
        }
  ...
  ctx.tools.register(defineTool({
    ...
    execute(args, exec) {
      ...
      if (!foldActive(exec.agent.session.events)) {
        exec.agent.session.append('rheostat/active', { active: true }, { ignorable: true })
      }
      exec.agent.session.append('rheostat/position', { position }, { ignorable: true })
