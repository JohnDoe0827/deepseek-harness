...
describe('/rheostat', () => {
  ...
  it('marks dial-state events ignorable so out-of-repo readers keep the session loadable', async () => {
    // `rheostat/position` and `rheostat/active` are plugin-owned event types
    // outside a stock harness's generated vocabulary; the ignorable envelope
    // marker is what lets such a reader skip them instead of refusing the
    // whole session log.
    const ctx = await setupWithCommands()
    const agent = await agentWithSession(ctx, 'command-ignorable')
    await ctx.commands.execute(agent, '/rheostat off', testSignal)
    await ctx.commands.execute(agent, '/rheostat 0.25', testSignal)
    const dial = agent.session.events.filter(event =>
      event.type === 'rheostat/position' || event.type === 'rheostat/active')
    expect(dial.length).toBeGreaterThan(0)
    for (const event of dial) {
      expect(event.ignorable).toBe(true)
    }
    const setResult = await ctx.tools.execute({
      signal: testSignal,
      callId: CallId('set-ignorable'),
      name: rheostat.RHEOSTAT_SET,
      arguments: { position: 0.8 },
      agent,
    })
    expect(setResult.isError).toBe(false)
    const lastPosition = agent.session.events.findLast(event => event.type === 'rheostat/position')
    expect(lastPosition?.type === 'rheostat/position' && lastPosition.ignorable).toBe(true)
  })
