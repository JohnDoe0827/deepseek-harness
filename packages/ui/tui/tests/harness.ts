...
export async function createTuiTestHarness<TerminalType extends Terminal, Exit extends (code: number) => void>(
  ...
  const agent: FakeAgent = {
    ...
    // The production Agent owns a live inbox and maintenance queue; this fake
    // drives only the surfaces the TUI touches, so the members are placeholders
    // that satisfy the type without claiming those behaviors.
    inbox: {} as never,
    ctx,
    sent,
    sentMessages,
