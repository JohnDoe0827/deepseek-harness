...
describe('pi-tui chat lifecycle and transcript', () => {
  it('restores durable goal phase without implying automatic continuation', async () => {
    ...
    const result = await setup({
      beforeMount(session) {
        // Establish the durable active goal first: the strict fold admits a
        // goal-sourced user message only as the next round after a goal/change.
        session.append('goal/change', change)
        session.append('user/message', createUserMessage({
          content: [{ type: 'text', text: `Goal ${change.goal.id} ${change.operation} (${change.goal.phase})` }],
          source: {
            kind: 'goal',
  ...
  it('badges queued steering on the prompt context and clears it as each drains', async () => {
    ...
    // Draining the last queued message returns the plain hint. The drain only
    // rewrites the prompt row; resize once so the unchanged editor row with its
    // empty-input hint is re-emitted under pi-tui's row-level diff.
    result.terminal.output = ''
  ...
  it('shows a live standalone compaction in the fixed status area', async () => {
    ...
    result.session.append('compaction/start', { compactionId: 'cmp-1' as never, turn: null })
    clock = 1_000
    result.terminal.output = ''
  ...
  it('ignores a numbered compaction bracket while the status line is idle', async () => {
    ...
    result.session.append('compaction/start', { compactionId: 'cmp-1' as never, turn: 1 })
    await tick()
  ...
  it('fades a closed standalone compaction back to the plain caret', async () => {
    ...
    const result = await setup({ omitInitialLifecycle: true, now: () => clock })
    clock = 1_000
    result.session.append('compaction/start', { compactionId: 'cmp-2' as never, turn: null })
    await tick()
    result.session.append('compaction/end', { compactionId: 'cmp-1' as never, turn: null })
    await tick()
  ...
  it('reports a failed standalone compaction when its live bracket closes', async () => {
    ...
    result.session.append('compaction/start', { compactionId: 'cmp-3' as never, turn: null })
    result.terminal.output = ''
    result.session.append('compaction/end', { compactionId: 'cmp-1' as never, turn: null, error: 'summary failed' })
    await tick()
  ...
  it('preserves live compaction progress across an idle status edge', async () => {
    let clock = 0
    const result = await setup({ omitInitialLifecycle: true, now: () => clock })
    result.session.append('compaction/start', { compactionId: 'cmp-4' as never, turn: null })
    clock = 1_000
    result.terminal.output = ''
    agentEvents(result.ctx, result.agent).emit('agent/status', { status: 'idle' })
    result.terminal.resize(result.terminal.columns + 1)
  ...
  it('keeps a running turn phase glyph ahead of standalone compaction', async () => {
    ...
    clock = 1_000
    result.terminal.output = ''
    result.session.append('compaction/start', { compactionId: 'cmp-5' as never, turn: null })
    await tick()

    expect(result.terminal.output).toContain('dsh ◍ ')
    expect(result.terminal.output).not.toContain('dsh ⊙ ')
    result.session.append('compaction/end', { compactionId: 'cmp-2' as never, turn: null })
    await tick()
    result.terminal.output = ''
    result.terminal.resize(result.terminal.columns + 1)
    await tick()
  ...
  it('treats duplicate live compaction starts as one owned bracket', async () => {
    ...
    try {
      ...
      result.session.append('compaction/start', { compactionId: 'cmp-6' as never, turn: null })
      clock = 1_000
      result.session.append('compaction/start', { compactionId: 'cmp-7' as never, turn: null })
      await tick()
      ...
      result.session.append('compaction/end', { compactionId: 'cmp-3' as never, turn: null })
      await tick()
  ...
  it('does not show compaction progress for a resumed orphaned start', async () => {
    const result = await setup({
      ...
      now: () => 1_000,
      beforeMount(session) {
        session.append('compaction/start', { compactionId: 'cmp-8' as never, turn: null })
      },
    })
  ...
  it('releases the live compaction timer and progress bit on dispose', async () => {
    ...
    try {
      ...
      intervalSpy.mockClear()
      clearIntervalSpy.mockClear()
      result.session.append('compaction/start', { compactionId: 'cmp-9' as never, turn: null })
      expect(intervalSpy).toHaveBeenCalledOnce()
  ...
  it('shows detailed session diagnostics while the agent is running', async () => {
    ...
    expect(result.terminal.output).toContain('You are an AI agent powered by')
  ...
  it('keeps a referenced prompt on admission when the agent is idle', async () => {
    const result = await setup({
      status: 'idle',
      omitInitialLifecycle: true,
      async configureContext(ctx) {
  ...
  it('releases the reference-admission wrapper on the ordinary allowed path', async () => {
    ...
    const unrelated = await agentEvents(result.ctx, result.agent).waterfall(
      'agent/pre-step',
      { messages: [unrelatedMessage], turn: 1, step: 1, signal: new AbortController().signal },
  ...
  it('discards the reference snapshot with its blocked or cancelled prompt', async () => {
    ...
    const passthrough = await agentEvents(result.ctx, result.agent).waterfall(
      'agent/pre-step',
      { messages: [differentMessage], turn: 1, step: 1, signal: new AbortController().signal },
    ...
    agentEvents(result.ctx, result.agent).emit('agent/inbox/discarded', { message: result.agent.sentMessages.at(-1)! })
    const afterDiscardMessage = result.agent.sentMessages.at(-1)!
    const afterDiscard = await agentEvents(result.ctx, result.agent).waterfall(
      'agent/pre-step',
      { messages: [afterDiscardMessage], turn: 1, step: 1, signal: new AbortController().signal },
      () => Promise.resolve({ kind: 'enter' as const, messages: [afterDiscardMessage] }),
    )
  ...
  it('keeps failed mention input and renders durable reference contexts as compact cards', async () => {
    ...
    // A source that is not an object with a string `kind` is rejected by the
    // session's append-time message-shape validation before the TUI renders.
    expect(() => result.session.append('user/message', createUserMessage({
      content: [{ type: 'text', text: 'invalid-shape' }],
      source: 'plain-string-source' as never,
    }), { surfaceOp: 'append' })).toThrow(/message has invalid source/)

    // Shape-valid but semantically invalid reference sources reach the
    // renderer, which must drop them without cards or crashes.
...
describe('terminal mounting', () => {
  it('starts immediately when the configured agent already exists', async () => {
    ...
    ctx.agents.register({
      ...
      inbox: {} as never, followup: () => {}, steer: () => {}, inject: () => {}, send: () => {}, cancel() {},
      whenIdle: () => Promise.resolve(), runMaintenance: async task => task(new AbortController().signal),
    })
    const terminal = new FakeTerminal()
